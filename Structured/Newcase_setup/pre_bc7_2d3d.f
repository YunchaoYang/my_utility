...
c        write(filename, '("bin_3d_",i3.3,".dat")') n
        ...
        if(idimen.eq.2) then 
           kl(n) = 0
           if(iltp.ne.il(n).or.jltp.ne.jl(n)) then
              print *, 'dimension data in datain does not match mesh'
              print *, 'il,jl should be ',iltp,jltp
              stop
           endif
        endif
        if(idimen.eq.3) then
